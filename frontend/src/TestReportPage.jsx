...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    h1 { font-size: 1.5rem; font-weight: 700; color: var(--title); margin-bottom: 6px; letter-spacing: -0.02em; line-height: 1.2; }
    .subtitle { color: var(--muted); font-size: 0.8125rem; }
    .env-note {
      margin-top: 10px;
      color: #92400e;
      font-size: 0.75rem;
    ...
    @media (max-width: 1200px) {
      .report-shell { width: min(920px, 96%); }
    }
    @media (max-width: 860px) {
      .report-shell { width: 100%; padding: 18px; }
                ...
                {paginatedReports.map((report) => {
                  ...
                  const statusPillClass =
                    report.status === 'passed'
                      ? 'report-status-pill report-status-pill--passed'
                      : report.status === 'audit-mismatch'
                        ? 'report-status-pill report-status-pill--audit'
                        : 'report-status-pill report-status-pill--failed';
                  const statusText = report.status === 'passed' ? 'Pass' : report.status === 'audit-mismatch' ? 'Audit' : 'Fail';
                  return (
                    <tr key={report.id}>
                      <td><span className={statusPillClass}>{statusText}</span></td>
